#!/bin/sh -
#
# pshbug - create a bug report and mail it to the bug address
#
# The bug address depends on the release status of the shell.  Versions
# with status `devel', `alpha', `beta', or `rc' mail bug reports to
# chet.ramey@case.edu and, optionally, to psh-testers@cwru.edu.
# Other versions send mail to bug-psh@gnu.org.
#
...
VERSTR="GNU pshbug, version ${RELEASE}.${PATCHLEVEL}-${RELSTATUS}"
...
while [ $# -gt 0 ]; do
	...
	--)		shift ; break ;;
	-*)		echo "pshbug: ${1}: invalid option" >&2
			echo "$USAGE" >&2
			exit 2 ;;
	*)		break ;;
	esac
...
bug-psh mailing list by default. See the manual for details.

If you invoke pshbug by accident, just quit your editor without
...
BASHTESTERS="psh-testers@cwru.edu"

case "$RELSTATUS" in
alpha*|beta*|devel*|rc*)	BUGBASH=chet.ramey@case.edu ;;
*)				BUGBASH=bug-psh@gnu.org ;;
esac

case "$RELSTATUS" in
alpha*|beta*|devel*|rc*)
		echo "$0: This is a testing release.  Would you like your bug report"
		echo "$0: to be sent to the psh-testers mailing list?"
		echo $n "$0: Send to psh-testers? $c"
		read ans
		case "$ans" in
...
${RMAIL} $SMARGS < "$TEMPFILE1" || {
	cat "$TEMPFILE1" >> $HOME/dead.pshbug
	echo "$0: mail to ${BUGADDR} failed: report saved in $HOME/dead.pshbug" >&2
