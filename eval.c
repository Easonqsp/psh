     ...
     int i;
{
  ...
  psh_logout ();	/* run ~/.psh_logout if this is a login shell */
...
/* Call the YACC-generated parser and return the status of the parse.
   Input is read from the current input stream (psh_input).  yyparse
...
parse_command ()
{
  ...
  /* The tests are a combination of SHOULD_PROMPT() and prompt_again() 
     ...
     actually printed. */
  if (interactive && psh_input.type != st_string && parser_expanding_alias() == 0)
    {
#if defined (READLINE)
      if (no_line_editing || (psh_input.type == st_stdin && parser_will_prompt ()))
#endif
        execute_prompt_command ();
