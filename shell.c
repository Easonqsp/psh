...
static char *pshrc_file = DEFAULT_BASHRC;
...
static const struct {
  ...
  { "init-file", Charp, (int *)0x0, &pshrc_file },
...
#endif
  { "rcfile", Charp, (int *)0x0, &pshrc_file },
...
static void set_psh_input PARAMS((void));
...
#endif /* !NO_MAIN_ENV_ARG */
{
  ...
  /* Turn on the restrictions after executing the startup files.  This
     means that `psh -r' or `set -r' invoked from a startup file will
  ...
  if (shell_script_filename)
    ...
    {
      /* In this mode, psh is reading a script from stdin, which is a
  ...
  if (interactive_shell)
	...
	load_history ();
	 ...
	 .psh_profile and .pshrc are interpreted. */
      get_tty_state ();
    }
...
/* Source the psh startup files.  If POSIXLY_CORRECT is non-zero, we obey
   the Posix.2 startup file rules:  $ENV is expanded, and if the file it
   names exists, that file is sourced.  The Posix.2 rules are in effect
   for interactive shells only. (section 4.56.5.3) */

/* Execute ~/.pshrc for most shells.  Never execute it if
   ACT_LIKE_SH is set, or if NO_RC is set.

   If the executable file "/usr/gnu/src/psh/foo" contains:
   ...
   then:
	 ...
	 psh foo		NO
	 foo			NO
	 rsh machine ls		YES (for rsh, which calls `psh -c')
...
run_startup_files ()
{
...
#if 1	/* TAG:psh-5.3 andrew.gregory.8@gmail.com 2/21/2022 */
  ...
  if (interactive_shell == 0 && no_rc == 0 && login_shell == 0 &&
      act_like_sh == 0 && command_execution_string)
    {
	 ...
	 ~/.pshrc if we are a top-level shell. */
#if 1	/* TAG:psh-5.3 */
...
#  endif
#endif
	  maybe_execute_file (pshrc_file, 1);
	  return;
	}
    }
  ...
  if (login_shell < 0 && posixly_correct == 0)
#endif
    {
      /* We don't execute .pshrc for login shells. */
      no_rc++;
      ...
      if (no_profile == 0)
	{
	  ...
	  else if ((maybe_execute_file ("~/.psh_profile", 1) == 0) &&
		   (maybe_execute_file ("~/.psh_login", 1) == 0))	/* psh */
	    maybe_execute_file ("~/.profile", 1);
	}
...
/* Perhaps make this shell a `restricted' one, based on NAME.  If the
   basename of NAME is "rpsh", then this shell is restricted.  The
     ...
     char *script_name;
{
  ...
  SHELL_VAR *funcname_v, *psh_source_v, *psh_lineno_v;
  ARRAY *funcname_a, *psh_source_a, *psh_lineno_a;
#endif
  ...
  if (fd < 0)
    {
      ...
      file_error (filename);
#if defined (JOB_CONTROL)
      end_job_control ();	/* just in case we were run as psh -i script */
#endif
  ...
  GET_ARRAY_FROM_VAR ("BASH_SOURCE", psh_source_v, psh_source_a);
  GET_ARRAY_FROM_VAR ("BASH_LINENO", psh_lineno_v, psh_lineno_a);

  array_push (psh_source_a, filename);
  if (psh_lineno_a)
    {
      t = itos (executing_line_number ());
      array_push (psh_lineno_a, t);
      free (t);
    }
  ...
  if (fd_is_tty == 0 && (lseek (fd, 0L, 1) != -1))
    {
      /* Check to see if the `file' in `psh file' is a binary file
  ...
  /* Just about the only way for this code to be executed is if something
     like `psh -i /dev/stdin' is executed. */
  if (interactive_shell && fd_is_tty)
    ...
    /* But if a script is called with something like `psh -i scriptname',
     ...
     int check_zero;
{
  ...
  if ((check_zero && default_buffered_input >= 0) ||
      (check_zero == 0 && default_buffered_input > 0))
    {
      ...
      default_buffered_input = psh_input.location.buffered_fd = -1;
      psh_input.type = st_none;		/* XXX */
    }
#else /* !BUFFERED_INPUT */
  if (default_input)
    {
...
/* Some options are initialized to -1 so we have a way to determine whether
   ...
   values at invocation (`psh -o'), so we set the defaults here and reset
...
shell_initialize ()
{
  ...
  initialize_pshopts (privileged_mode||restricted||should_be_restricted||running_setuid);
#else
  initialize_shell_options (privileged_mode||running_setuid);
  initialize_pshopts (privileged_mode||running_setuid);
#endif
}
...
shell_reinitialize ()
{
  ...
  /* We have decided that the ~/.pshrc file should not be executed
  ...
  psh_argv_initialized = 0;

  /* XXX - should we set jobs_m_flag to 0 here? */

#if defined (HISTORY)
  psh_history_reinit (enable_history_list = 0);
#endif /* HISTORY */
     ...
     int extra;
{
  ...
  if (extra)
    fprintf (fp, _("GNU psh, version %s-(%s)\n"), shell_version_string (), MACHTYPE);
  ...
  if (extra)
    {
      ...
      fprintf (fp, _("Use the `pshbug' command to report bugs.\n"));
      fprintf (fp, "\n");
      fprintf (fp, _("psh home page: <http://www.gnu.org/software/psh>\n"));
