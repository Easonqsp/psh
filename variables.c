...
static char *get_psh_name PARAMS((void));
...
static SHELL_VAR *get_pshpid PARAMS((SHELL_VAR *));

static SHELL_VAR *get_psh_argv0 PARAMS((SHELL_VAR *));
static SHELL_VAR *assign_psh_argv0 PARAMS((SHELL_VAR *, char *, arrayind_t, char *));
...
#  if defined (DEBUGGER)
static SHELL_VAR *get_pshargcv PARAMS((SHELL_VAR *));
#  endif
...
#  endif
  ...
  if (remember_on_history)
    {
      name = psh_tilde_expand (posixly_correct ? "~/.sh_history" : "~/.psh_history", 0);
...
static SHELL_VAR *
assign_psh_argv0 (var, value, unused, key)
     SHELL_VAR *var;
     char *value;
     arrayind_t unused;
     char *key;
...
set_argv0 ()
{
  ...
  if (v && imported_p (v))
    assign_psh_argv0 (v, value_cell (v), 0, 0);
}
     ...
     SHELL_VAR *var;
{
  ...
  if (rl_completer_word_break_characters == 0 && psh_readline_initialized == 0)
...
initialize_dynamic_variables ()
{
  ...
  INIT_DYNAMIC_VAR ("BASH_ARGV0", (char *)NULL, get_psh_argv0, assign_psh_argv0);

  INIT_DYNAMIC_VAR ("BASH_COMMAND", (char *)NULL, get_psh_command, (sh_var_assign_func_t *)NULL);
  ...
  INIT_DYNAMIC_VAR ("BASHPID", (char *)NULL, get_pshpid, null_assign);
  VSETATTR (v, att_integer);
...
#  if defined (DEBUGGER)
  v = init_dynamic_array_var ("BASH_ARGC", get_pshargcv, null_array_assign, att_noassign|att_nounset);
  v = init_dynamic_array_var ("BASH_ARGV", get_pshargcv, null_array_assign, att_noassign|att_nounset);
#  endif /* DEBUGGER */
	  ...
	  if (tname && (tentry = find_variable_noref (tname)) && nameref_p (tentry))
	    {
	      ...
	      FREE (value_cell (tentry));		/* XXX - psh-4.3 compat */
     ...
     WORD_LIST *list;
{
  ...
  SHELL_VAR *psh_argv_v, *psh_argc_v;
  ARRAY *psh_argv_a, *psh_argc_a;
  WORD_LIST *l;
  arrayind_t i;
  char *t;

  GET_ARRAY_FROM_VAR ("BASH_ARGV", psh_argv_v, psh_argv_a);
  GET_ARRAY_FROM_VAR ("BASH_ARGC", psh_argc_v, psh_argc_a);

  for (l = list, i = 0; l; l = l->next, i++)
    array_push (psh_argv_a, l->word->word);

  t = itos (i);
  array_push (psh_argc_a, t);
  free (t);
...
pop_args ()
{
  ...
  ce = array_unshift_element (psh_argc_a);
  if (ce == 0 || legal_number (element_value (ce), &i) == 0)
    i = 0;

  for ( ; i > 0; i--)
    array_pop (psh_argv_a);
