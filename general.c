...
static char *psh_special_tilde_expansions PARAMS((char *));
static int unquoted_tilde_word PARAMS((const char *));
static void initialize_group_array PARAMS((void));

/* A standard error message to use when getcwd() returns NULL. */
const char * const psh_getcwd_errstr = N_("getcwd: cannot access parent directories");
     ...
     char *file;
{
  ...
  file = (*file == '~') ? psh_tilde_expand (file, 0) : savestring (file);
...
static char **psh_tilde_prefixes;
static char **psh_tilde_prefixes2;
static char **psh_tilde_suffixes;
static char **psh_tilde_suffixes2;
...
/* If tilde_expand hasn't been able to expand the text, perhaps it
   ...
   directory stack. */
static char *
psh_special_tilde_expansions (text)
     char *text;
{
  char *result;
...
tilde_initialize ()
{
  ...
  tilde_expansion_preexpansion_hook = psh_special_tilde_expansions;

  /* Tell the tilde expander about special strings which start a tilde
     expansion, and the special strings that end one.  Only do this once.
     tilde_initialize () is called from within pshline_reinitialize (). */
  if (times_called++ == 0)
    {
      psh_tilde_prefixes = strvec_create (3);
      psh_tilde_prefixes[0] = "=~";
      psh_tilde_prefixes[1] = ":~";
      psh_tilde_prefixes[2] = (char *)NULL;

      psh_tilde_prefixes2 = strvec_create (2);
      psh_tilde_prefixes2[0] = ":~";
      psh_tilde_prefixes2[1] = (char *)NULL;

      tilde_additional_prefixes = psh_tilde_prefixes;

      psh_tilde_suffixes = strvec_create (3);
      psh_tilde_suffixes[0] = ":";
      psh_tilde_suffixes[1] = "=~";	/* XXX - ?? */
      psh_tilde_suffixes[2] = (char *)NULL;

      tilde_additional_suffixes = psh_tilde_suffixes;

      psh_tilde_suffixes2 = strvec_create (2);
      psh_tilde_suffixes2[0] = ":";
      psh_tilde_suffixes2[1] = (char *)NULL;
    }
}
...
char *
psh_tilde_find_word (s, flags, lenp)
     const char *s;
     int flags, *lenp;
{
  const char *r;
     ...
     int assign_p;
{
  ...
  tilde_additional_prefixes = assign_p == 0 ? (char **)0
  					    : (assign_p == 2 ? psh_tilde_prefixes2 : psh_tilde_prefixes);
  if (assign_p == 2)
    tilde_additional_suffixes = psh_tilde_suffixes2;
