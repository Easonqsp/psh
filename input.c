...
set_psh_input_fd (fd)
     int fd;
{
  if (psh_input.type == st_bstream)
    psh_input.location.buffered_fd = fd;
...
fd_is_psh_input (fd)
     int fd;
{
  if (psh_input.type == st_bstream && psh_input.location.buffered_fd == fd)
    return 1;
...
/* Save the buffered stream corresponding to file descriptor FD (which psh
     ...
     int fd, new_fd;
{
  ...
  /* Now take care of duplicating the file descriptor that psh is
     using for input, so we can reinitialize it later. */
  nfd = (new_fd == -1) ? fcntl (fd, F_DUPFD, 10) : new_fd;
  if (nfd == -1)
    {
      if (fcntl (fd, F_GETFD, 0) == 0)
	sys_error (_("cannot allocate new file descriptor for psh input from fd %d"), fd);
      return -1;
    }
  ...
  if (nfd < nbuffers && buffers[nfd])
    {
	 ...
	 descriptor?  Free up the buffer and report the error. */
      internal_error (_("save_psh_input: buffer already exists for new fd %d"), nfd);
      if (buffers[nfd]->b_flag & B_SHAREDBUF)
	buffers[nfd]->b_buffer = (char *)NULL;
      free_buffered_stream (buffers[nfd]);
    }

  /* Reinitialize psh_input.location. */
  if (psh_input.type == st_bstream)
    {
      psh_input.location.buffered_fd = nfd;
...
/* Check that file descriptor FD is not the one that psh is currently
   using to read input from a script.  FD is about to be duplicated onto,
   which means that the kernel will close it for us.  If FD is the psh
   ...
   allocate a new file descriptor to use for psh input, and re-initialize
   the buffered stream.  Make sure the file descriptor used to save psh
   input is set close-on-exec. Returns 0 on success, -1 on failure.  This
   works only if fd is > 0 -- if fd == 0 and psh is reading input from
...
check_psh_input (fd)
     int fd;
{
  if (fd_is_psh_input (fd))
    {
      if (fd > 0)
	return ((save_psh_input (fd, -1) == -1) ? -1 : 0);
      else if (fd == 0)
     ...
     int fd1, fd2;
{
  ...
  /* If FD2 is the file descriptor psh is currently using for shell input,
  ...
  is_psh_input = (psh_input.type == st_bstream) &&
		  (psh_input.location.buffered_fd == fd2);
  ...
  if (fd_is_psh_input (fd1) || (buffers[fd1] && (buffers[fd1]->b_flag & B_SHAREDBUF)))
...
buffered_getchar ()
{
  ...
  if (psh_input.location.buffered_fd < 0 || buffers[psh_input.location.buffered_fd] == 0)
    return EOF;

#if !defined (DJGPP)
  return (bufstream_getc (buffers[psh_input.location.buffered_fd]));
#else
  /* On DJGPP, ignore \r. */
  int ch;
  while ((ch = bufstream_getc (buffers[psh_input.location.buffered_fd])) == '\r')
    ;
  return ch;
#endif
}
...
buffered_ungetchar (c)
     int c;
{
  return (bufstream_ungetc (c, buffers[psh_input.location.buffered_fd]));
}
