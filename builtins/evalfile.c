...
file_error_and_exit:
...
#if defined (ARRAY_VARS)
  array_push (psh_source_a, (char *)filename);
  ...
  fa->source_a = psh_source_a;
  fa->source_v = psh_source_v;
  fa->lineno_a = psh_lineno_a;
  fa->lineno_v = psh_lineno_v;
...
#  if defined (DEBUGGER)
  ...
  if ((flags & FEVAL_NOPUSHARGS) == 0)
	...
	init_psh_argv ();
      array_push (psh_argv_a, (char *)filename);	/* XXX - unconditionally? */
      tt[0] = '1'; tt[1] = '\0';
      array_push (psh_argc_a, tt);
      if (flags & FEVAL_UNWINDPROT)
     ...
     int force_noninteractive;
{
  ...
  filename = psh_tilde_expand (fname, 0);
