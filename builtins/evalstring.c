...
can_optimize_connection (command)
     COMMAND *command;
{
  return (*psh_input.location.string == '\0' &&
	  parser_expanding_alias () == 0 &&
...
/* Parse and execute the commands in STRING.  Returns whatever
   ...
   are:
   	...
   	(flags & SEVAL_NOHIST) -> call psh_history_disable ()
     ...
     int flags;
{
  ...
  while (*(psh_input.location.string) || parser_expanding_alias ())
    {
      ...
      if (parse_command () == 0)
	{
	  ...
	  if ((flags & SEVAL_PARSEONLY) || (interactive_shell == 0 && read_but_dont_execute))
		...
		command->flags |= CMD_IGNORE_RETURN;
	      ...
	      /*
	       * IF
	       *   we were invoked as `psh -c' (startup_state == 2) AND
     ...
     char **endp;
{
  ...
  with_input_from_string (string, from_file);
  ostring = psh_input.location.string;
  while (*(psh_input.location.string))		/* XXX - parser_expanding_alias () ? */
    {
      command = (COMMAND *)NULL;
...
out:
  ...
  nc = psh_input.location.string - ostring;
  if (endp)
    *endp = psh_input.location.string;
