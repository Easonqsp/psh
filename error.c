...
extern const char * const psh_badsub_errmsg;
#endif
...
#define MAINTAINER "psh-maintainers@gnu.org"
#endif
...
get_name_for_error ()
{
  ...
  if (interactive_shell == 0)
    {
#if defined (ARRAY_VARS)
      psh_source_v = find_variable ("BASH_SOURCE");
      if (psh_source_v && array_p (psh_source_v) &&
	  (psh_source_a = array_cell (psh_source_v)))
	name = array_reference (psh_source_a, 0);
...
#endif
{
  ...
  if (tracefp == NULL)
    tracefp = fopen("/tmp/psh-trace.log", "a+");
...
err_badarraysub (s)
     const char *s;
{
  report_error ("%s: %s", s, _(psh_badsub_errmsg));
}
#endif
