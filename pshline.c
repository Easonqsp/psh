/* pshline.c -- Bash's interface to the readline library. */
...
extern int psh_brace_completion PARAMS((int, int));
...
static int psh_ignore_filenames PARAMS((char **));
static int psh_ignore_everything PARAMS((char **));
static int psh_progcomp_ignore_filenames PARAMS((char **));
...
static int psh_forward_shellword PARAMS((int, int));
static int psh_backward_shellword PARAMS((int, int));
static int psh_kill_shellword PARAMS((int, int));
static int psh_backward_kill_shellword PARAMS((int, int));
static int psh_transpose_shellwords PARAMS((int, int));

static int psh_spell_correct_shellword PARAMS((int, int));

/* Helper functions for Readline. */
static char *restore_tilde PARAMS((char *, char *));
static char *maybe_restore_tilde PARAMS((char *, char *));

static char *psh_filename_rewrite_hook PARAMS((char *, int));

static void psh_directory_expansion PARAMS((char **));
static int psh_filename_stat_hook PARAMS((char **));
static int psh_command_name_stat_hook PARAMS((char **));
static int psh_directory_completion_hook PARAMS((char **));
static int filename_completion_ignore PARAMS((char **));
static int psh_push_line PARAMS((void));
...
static int psh_dabbrev_expand PARAMS((int, int));
...
static char *psh_dequote_filename PARAMS((char *, int));
static char *quote_word_break_chars PARAMS((char *));
static int psh_check_expchar PARAMS((char *, int, int *, int *));
static void set_filename_quote_chars PARAMS((int, int, int));
static void set_filename_bstab PARAMS((const char *));
static char *psh_quote_filename PARAMS((char *, int, char *));
...
static int psh_event_hook PARAMS((void));
...
static int psh_specific_completion PARAMS((int, rl_compentry_func_t *));

static int psh_complete_filename_internal PARAMS((int));
static int psh_complete_username_internal PARAMS((int));
static int psh_complete_hostname_internal PARAMS((int));
static int psh_complete_variable_internal PARAMS((int));
static int psh_complete_command_internal PARAMS((int));

static int psh_complete_filename PARAMS((int, int));
static int psh_possible_filename_completions PARAMS((int, int));
static int psh_complete_username PARAMS((int, int));
static int psh_possible_username_completions PARAMS((int, int));
static int psh_complete_hostname PARAMS((int, int));
static int psh_possible_hostname_completions PARAMS((int, int));
static int psh_complete_variable PARAMS((int, int));
static int psh_possible_variable_completions PARAMS((int, int));
static int psh_complete_command PARAMS((int, int));
static int psh_possible_command_completions PARAMS((int, int));

static int completion_glob_pattern PARAMS((char *));
static char *glob_complete_word PARAMS((const char *, int));
static int psh_glob_completion_internal PARAMS((int));
static int psh_glob_complete_word PARAMS((int, int));
static int psh_glob_expand_word PARAMS((int, int));
static int psh_glob_list_expansions PARAMS((int, int));
...
static char *psh_completer_word_break_characters = " \t\n\"'@><=;|&(:";
static char *psh_nohostname_word_break_characters = " \t\n\"'><=;|&(:";
/* )) */
...
/* What kind of quoting is performed by psh_quote_filename:
...
/* Flag values for the final argument to psh_default_completion */
...
{
  rl_completer_word_break_characters = perform_hostname_completion ? savestring (psh_completer_word_break_characters) : savestring (psh_nohostname_word_break_characters);
}
     ...
     int on_or_off;
{
  ...
  /* If this is the first time this has been called
     (psh_readline_initialized == 0), use the sames values as before, but
     allocate new memory for rl_completer_word_break_characters. */

  if (psh_readline_initialized == 0 &&
      (rl_completer_word_break_characters == 0 || 
       rl_completer_word_break_characters == rl_basic_word_break_characters))
    {
      if (on_or_off)
	rl_completer_word_break_characters = savestring (psh_completer_word_break_characters);
      else
	rl_completer_word_break_characters = savestring (psh_nohostname_word_break_characters);
    }
  else
    {
...
initialize_readline ()
{
  ...
  rl_add_defun ("shell-forward-word", psh_forward_shellword, -1);
  rl_add_defun ("shell-backward-word", psh_backward_shellword, -1);
  rl_add_defun ("shell-kill-word", psh_kill_shellword, -1);
  rl_add_defun ("shell-backward-kill-word", psh_backward_kill_shellword, -1);
  rl_add_defun ("shell-transpose-words", psh_transpose_shellwords, -1);

  rl_add_defun ("spell-correct-word", psh_spell_correct_shellword, -1);
  rl_bind_key_if_unbound_in_map ('s', psh_spell_correct_shellword, emacs_ctlx_keymap);
...
#  endif
...
#if defined (BRACE_COMPLETION)
  rl_add_defun ("complete-into-braces", psh_brace_completion, -1);
#endif

#if defined (SPECIFIC_COMPLETION_FUNCTIONS)
  rl_add_defun ("complete-filename", psh_complete_filename, -1);
  rl_add_defun ("possible-filename-completions", psh_possible_filename_completions, -1);
  rl_add_defun ("complete-username", psh_complete_username, -1);
  rl_add_defun ("possible-username-completions", psh_possible_username_completions, -1);
  rl_add_defun ("complete-hostname", psh_complete_hostname, -1);
  rl_add_defun ("possible-hostname-completions", psh_possible_hostname_completions, -1);
  rl_add_defun ("complete-variable", psh_complete_variable, -1);
  rl_add_defun ("possible-variable-completions", psh_possible_variable_completions, -1);
  rl_add_defun ("complete-command", psh_complete_command, -1);
  rl_add_defun ("possible-command-completions", psh_possible_command_completions, -1);
  rl_add_defun ("glob-complete-word", psh_glob_complete_word, -1);
  rl_add_defun ("glob-expand-word", psh_glob_expand_word, -1);
  rl_add_defun ("glob-list-expansions", psh_glob_list_expansions, -1);
#endif

  rl_add_defun ("dynamic-complete-history", dynamic_complete_history, -1);
  rl_add_defun ("dabbrev-expand", psh_dabbrev_expand, -1);
...
#if defined (BRACE_COMPLETION)
  rl_bind_key_if_unbound_in_map ('{', psh_brace_completion, emacs_meta_keymap); /*}*/
#endif /* BRACE_COMPLETION */

#if defined (SPECIFIC_COMPLETION_FUNCTIONS)
  rl_bind_key_if_unbound_in_map ('/', psh_complete_filename, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map ('/', psh_possible_filename_completions, emacs_ctlx_keymap);
  ...
  if (func == 0 || func == rl_tilde_expand)
    rl_bind_keyseq_in_map (kseq, psh_complete_username, emacs_meta_keymap);

  rl_bind_key_if_unbound_in_map ('~', psh_possible_username_completions, emacs_ctlx_keymap);

  rl_bind_key_if_unbound_in_map ('@', psh_complete_hostname, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map ('@', psh_possible_hostname_completions, emacs_ctlx_keymap);

  rl_bind_key_if_unbound_in_map ('$', psh_complete_variable, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map ('$', psh_possible_variable_completions, emacs_ctlx_keymap);

  rl_bind_key_if_unbound_in_map ('!', psh_complete_command, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map ('!', psh_possible_command_completions, emacs_ctlx_keymap);

  rl_bind_key_if_unbound_in_map ('g', psh_glob_complete_word, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map ('*', psh_glob_expand_word, emacs_ctlx_keymap);
  rl_bind_key_if_unbound_in_map ('g', psh_glob_list_expansions, emacs_ctlx_keymap);
  ...
  rl_filename_rewrite_hook = psh_filename_rewrite_hook;

  rl_filename_stat_hook = psh_filename_stat_hook;
...
#  endif
  ...
  rl_bind_key_in_map ('\\', psh_vi_complete, vi_movement_keymap);
  rl_bind_key_in_map ('*', psh_vi_complete, vi_movement_keymap);
  rl_bind_key_in_map ('=', psh_vi_complete, vi_movement_keymap);
#endif
  ...
  rl_filename_quoting_function = psh_quote_filename;
  rl_filename_dequoting_function = psh_dequote_filename;
  rl_char_is_quoted_p = char_is_quoted;

  /* Add some default bindings for the "shellwords" functions, roughly
     parallelling the default word bindings in emacs mode. */
  rl_bind_key_if_unbound_in_map (CTRL('B'), psh_backward_shellword, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map (CTRL('D'), psh_kill_shellword, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map (CTRL('F'), psh_forward_shellword, emacs_meta_keymap);
  rl_bind_key_if_unbound_in_map (CTRL('T'), psh_transpose_shellwords, emacs_meta_keymap);
  ...
  psh_readline_initialized = 1;
}

void
pshline_reinitialize ()
{
  psh_readline_initialized = 0;
}

void
pshline_set_event_hook ()
{
  rl_signal_event_hook = psh_event_hook;
}

void
pshline_reset_event_hook ()
{
  ...
  if (rl_explicit_arg)
  ...
  else
    {
	 ...
	 finished with the command, so we should not ignore the last command */
      ...
      psh_add_history (rl_line_buffer);
...
static int
psh_forward_shellword (count, key)
     int count, key;
{
  size_t slen;
  int c, p;
  ...
  if (count < 0)
    return (psh_backward_shellword (-count, key));
...
static int
psh_backward_shellword (count, key)
     int count, key;
{
  size_t slen;
  int c, p, prev_p;
  ...
  if (count < 0)
    return (psh_forward_shellword (-count, key));
...
static int
psh_kill_shellword (count, key)
     int count, key;
{
  int p;

  if (count < 0)
    return (psh_backward_kill_shellword (-count, key));
...
static int
psh_backward_kill_shellword (count, key)
     int count, key;
{
  int p;

  if (count < 0)
    return (psh_kill_shellword (-count, key));
...
static int
psh_transpose_shellwords (count, key)
     int count, key;
{
  char *word1, *word2;
  ...
  psh_forward_shellword (count, key);
  w2_end = rl_point;
  psh_backward_shellword (1, key);
  w2_beg = rl_point;
  psh_backward_shellword (count, key);
  w1_beg = rl_point;
  psh_forward_shellword (1, key);
  w1_end = rl_point;
...
static int
psh_spell_correct_shellword (count, key)
     int count, key;
{
  ...
  while (count)
	...
	break;
      ...
      if (count)
	psh_forward_shellword (1, key);		/* XXX */
    }
     ...
     int start, end;
{
  ...
  if (matches == 0)
	...
	dflags |= DEFCOMP_CMDPOS;
      matches = psh_default_completion (text, start, end, qc, dflags);
    }
...
char **
psh_default_completion (text, start, end, qc, compflags)
     const char *text;
     int start, end, qc, compflags;
{
  ...
  if (matches == 0 && (compflags & DEFCOMP_CMDPOS))
	...
	{
	  matches = (char **)NULL;
	  rl_ignore_some_completions_function = psh_ignore_everything;
	}
      else
	{
#define CMD_IS_DIR(x)	(absolute_pathname(x) == 0 && absolute_program(x) == 0 && *(x) != '~' && test_for_directory (x))
	  ...
	  if (matches == (char **)NULL)
	    rl_ignore_some_completions_function = psh_ignore_filenames;
...
static int
psh_command_name_stat_hook (name)
     char **name;
{
  char *cname, *result;
  ...
  if (absolute_program (*name))
    return (psh_filename_stat_hook (name));
...
#else
  c = 0;
  f = psh_quote_filename ((char *)filename, SINGLE_MATCH, &c);
#endif
  psh_directory_completion_hook (&f);
     ...
     int state;
{
  ...
  if (state == 0)
    {
      rl_filename_stat_hook = psh_command_name_stat_hook;
      ...
      if (globpat || absolute_program (hint_text))
	{
	  ...
	  if (*hint_text == '~')
	    {
	      hint = psh_tilde_expand (hint_text, 0);
		...
		*temp = 0;
	  ...
	  else if (dircomplete_expand)
	    {
	      ...
	      psh_directory_completion_hook (&hint);
	    }
	  else
	    hint = savestring (hint_text);
	  ...
	  if (rl_completion_found_quote && rl_completion_quote_character == 0)
	    {
	      dequoted_hint = psh_dequote_filename (hint, 0);
	      free (hint);
	      hint = dequoted_hint;
	    }
 ...
 outer:
  ...
  if (istate == 0)
      ...
      if (*current_path == '~')
	{
	  ...
	  t = psh_tilde_expand (current_path, 0);
	  free (current_path);
	  current_path = t;
	}
 ...
 inner:
  ...
  if (val == 0)
      ...
      else
	{
	 ...
	 psh execution code won't find executables in directories which
 ...
 */
char *
psh_servicename_completion_function (text, state)
     const char *text;
     int state;
{
 ...
 */
char *
psh_groupname_completion_function (text, state)
     const char *text;
     int state;
{
     ...
     const char *name;
{
  ...
  fn = (*name == '~') ? psh_tilde_expand (name, 0) : savestring (name);
  psh_filename_stat_hook (&fn);
  r = file_isdir (fn);
  free (fn);
...
static int
psh_progcomp_ignore_filenames (names)
     char **names;
{
     ...
     char *val, *directory_part;
{
  ...
  dh2 = directory_part ? psh_dequote_filename (directory_part, 0) : 0;
  psh_directory_expansion (&dh2);
  dl2 = strlen (dh2);

  expdir = psh_tilde_expand (directory_part, 0);
  xl = strlen (expdir);
...
static void
psh_directory_expansion (dirname)
     char **dirname;
{
  char *d, *nd;
  ...
  if ((rl_directory_rewrite_hook) && (*rl_directory_rewrite_hook) (&d))
    ...
    {
      nd = psh_dequote_filename (d, rl_completion_quote_character);
      free (*dirname);
      free (d);
      *dirname = nd;
    }
...
static char *
psh_filename_rewrite_hook (fname, fnlen)
     char *fname;
     int fnlen;
{
  char *conv;
...
set_directory_hook ()
{
  if (dircomplete_expand)
    {
      rl_directory_completion_hook = psh_directory_completion_hook;
      rl_directory_rewrite_hook = (rl_icppfunc_t *)0;
    }
  else
    {
      rl_directory_rewrite_hook = psh_directory_completion_hook;
     ...
     int should_dequote;
{
  ...
  new_dirname = should_dequote ? psh_dequote_filename ((char *)dirname, rl_completion_quote_character) : savestring (dirname);
...
static int
psh_filename_stat_hook (dirname)
     char **dirname;
{
  ...
  /* This is very similar to the code in psh_directory_completion_hook below,
...
static int
psh_directory_completion_hook (dirname)
     char **dirname;
{
  char *local_dirname, *new_dirname, *t;
  ...
  should_expand_dirname = psh_check_expchar (local_dirname, 1, &nextch, &closer);
  ...
  if (should_expand_dirname)  
  ...
  else 
    {
      ...
      new_dirname = psh_dequote_filename (local_dirname, rl_completion_quote_character);
  ...
  /* This is test as it was in psh-4.2: skip relative pathnames in current
     ...
     int count, key;
{
  ...
  if (rl_last_func == psh_dabbrev_expand)
    rl_last_func = rl_menu_complete;
  r = rl_menu_complete (count, key);
  dabbrev_expand_active = 0;

  rl_last_func = psh_dabbrev_expand;
...
#if defined (SPECIFIC_COMPLETION_FUNCTIONS)
static int
psh_complete_username (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_username_internal (rl_completion_mode (psh_complete_username));
}

static int
psh_possible_username_completions (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_username_internal ('?');
}

static int
psh_complete_username_internal (what_to_do)
     int what_to_do;
{
  return psh_specific_completion (what_to_do, rl_username_completion_function);
}

static int
psh_complete_filename (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_filename_internal (rl_completion_mode (psh_complete_filename));
}

static int
psh_possible_filename_completions (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_filename_internal ('?');
}

static int
psh_complete_filename_internal (what_to_do)
     int what_to_do;
{
  rl_compentry_func_t *orig_func;
  rl_completion_func_t *orig_attempt_func;
...
static int
psh_complete_hostname (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_hostname_internal (rl_completion_mode (psh_complete_hostname));
}

static int
psh_possible_hostname_completions (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_hostname_internal ('?');
}

static int
psh_complete_variable (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_variable_internal (rl_completion_mode (psh_complete_variable));
}

static int
psh_possible_variable_completions (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_variable_internal ('?');
}

static int
psh_complete_command (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_command_internal (rl_completion_mode (psh_complete_command));
}

static int
psh_possible_command_completions (ignore, ignore2)
     int ignore, ignore2;
{
  return psh_complete_command_internal ('?');
}

static int
psh_complete_hostname_internal (what_to_do)
     int what_to_do;
{
  return psh_specific_completion (what_to_do, hostname_completion_function);
}

static int
psh_complete_variable_internal (what_to_do)
     int what_to_do;
{
  return psh_specific_completion (what_to_do, variable_completion_function);
}

static int
psh_complete_command_internal (what_to_do)
     int what_to_do;
{
  return psh_specific_completion (what_to_do, command_word_completion_function);
}
...
static int
psh_glob_completion_internal (what_to_do)
     int what_to_do;
{
  return psh_specific_completion (what_to_do, glob_complete_word);
}
...
static char *
psh_glob_quote_filename (s, rtype, qcp)
     char *s;
     int rtype;
     char *qcp;
{
  if (globorig && qcp && *qcp == '\0' && STREQ (s, globorig))
    ...
    return (psh_quote_filename (s, rtype, qcp));
}

static int
psh_glob_complete_word (count, key)
     int count, key;
{
  int r;
  ...
  rl_filename_quoting_function = psh_glob_quote_filename;
  
  r = psh_glob_completion_internal (rl_completion_mode (psh_glob_complete_word));
...
static int
psh_glob_expand_word (count, key)
     int count, key;
{
  return psh_glob_completion_internal ('*');
}

static int
psh_glob_list_expansions (count, key)
     int count, key;
{
  return psh_glob_completion_internal ('?');
}
...
static int
psh_specific_completion (what_to_do, generator)
     int what_to_do;
...
/* Completion, from vi mode's point of view.  This is a modified version of
   rl_vi_complete which uses the psh globbing code to implement what POSIX
   specifies, which is to append a `*' and attempt filename generation (which
   has the side effect of expanding any globbing characters in the word). */
static int
psh_vi_complete (count, key)
     int count, key;
{
#if defined (SPECIFIC_COMPLETION_FUNCTIONS)
  int p, r;
  ...
  if (key == '*')	/* Expansion and replacement. */
    r = psh_glob_expand_word (count, key);
  else if (key == '=')	/* List possible completions. */
    r = psh_glob_list_expansions (count, key);
  else if (key == '\\')	/* Standard completion */
    r = psh_glob_complete_word (count, key);
  else
    r = rl_complete (0, key);
...
static char *
psh_dequote_filename (text, quote_char)
     char *text;
     int quote_char;
{
  char *ret, *p, *r;
...
static int
psh_check_expchar (dirname, need_closer, nextp, closerp)
     char *dirname;
     int need_closer;
     int *nextp, *closerp;
{
...
/* Quote a filename using double quotes, single quotes, or backslashes
   ...
   memory. */
static char *
psh_quote_filename (s, rtype, qcp)
     char *s;
     int rtype;
     char *qcp;
{
  ...
  if (*qcp == '\0' && cs == COMPLETE_BSQUOTE && dircomplete_expand == 0 &&
      (expchar = psh_check_expchar (s, 0, &nextch, &closer)) &&
      file_exists (s) == 0)
    {
      /* Usually this will have been set by psh_directory_completion_hook,
...
int
psh_execute_unix_command (count, key)
     int count;	/* ignored */
     int key;
{
  int type;
  ...
  if (cmd == 0 || type != ISMACR)
    {
      rl_crlf ();
      internal_error (_("psh_execute_unix_command: cannot find keymap for command"));
      rl_forced_update_display ();
      return 1;
    }
  ...
  save_parser_state (&ps);
  rl_clear_signals ();
  r = parse_and_execute (savestring (cmd), "psh_execute_unix_command", SEVAL_NOHIST);
  rl_set_signals ();
  restore_parser_state (&ps);
     ...
     char *line;
{
  ...
  rl_bind_keyseq_in_map (kseq, psh_execute_unix_command, kmap);
...
char **
psh_directory_completion_matches (text)
     const char *text;
{
  char **m1;
  char *dfn;
  ...
  if (rl_dispatching && rl_completion_found_quote == 0)
    dfn = psh_dequote_filename ((char *)text, qc);
  else
    dfn = (char *)text;
  ...
  /* We don't bother recomputing the lcd of the matches, because it will just
     ...
     later. */
  (void)psh_progcomp_ignore_filenames (m1);
  return m1;
}

char *
psh_dequote_text (text)
     const char *text;
{
  char *dtxt;
  int qc;
  ...
  dtxt = psh_dequote_filename ((char *)text, qc);
  return (dtxt);
}
