...
AC_CACHE_VAL(psh_cv_func_strtoimax,
[
  ...
  if test "$HAVE_STRTOIMAX" = 0 || test "$HAVE_DECL_STRTOIMAX" = 0 ; then
    psh_cv_func_strtoimax=no REPLACE_STRTOIMAX=1
  else
    psh_cv_func_strtoimax=yes
  fi
])
AC_MSG_RESULT($psh_cv_func_strtoimax)
if test $psh_cv_func_strtoimax = yes; then
AC_LIBOBJ(strtoimax)
fi
])
