  ...
  for (forcefork = 0, rd = redirects; rd; rd = rd->next)
    {
      ...
      /* Safety */
      forcefork += (rd->redirector.dest == 0 || fd_is_psh_input (rd->redirector.dest)) && (INPUT_REDIRECT (rd->instruction) || TRANSLATE_REDIRECT (rd->instruction) || rd->instruction == r_close_this);
    }
     ...
     int async, subshell;
{
  ...
  ARRAY *funcname_a;
  volatile ARRAY *psh_source_a;
  volatile ARRAY *psh_lineno_a;
  struct func_array_state *fa;
#endif
  ...
  array_push ((ARRAY *)psh_source_a, sfile);
  lineno = GET_LINE_NUMBER ();
  t = itos (lineno);
  array_push ((ARRAY *)psh_lineno_a, t);
  free (t);
#endif

#if defined (ARRAY_VARS)
  fa = (struct func_array_state *)xmalloc (sizeof (struct func_array_state));
  fa->source_a = (ARRAY *)psh_source_a;
  fa->source_v = psh_source_v;
  fa->lineno_a = (ARRAY *)psh_lineno_a;
  ...
  */
  ...
  if (pid == 0)
      ...
      if (async)
	{
	 ...
	 because of the way psh does pipes; fds_to_close is a
