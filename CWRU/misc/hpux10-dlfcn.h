 ...
 *
 * According to the ld manual page, you need to link psh with -dld and add
...
char *psh_global_sym_addr;

#define dlopen(file,mode) (void *)shl_load((file), (mode), 0L)

#define dlclose(handle) shl_unload((shl_t)(handle))

#define dlsym(handle,name) (psh_global_sym_addr=0,shl_findsym((shl_t *)&(handle),name,TYPE_UNDEFINED,&psh_global_sym_addr), (void *)psh_global_sym_addr)
