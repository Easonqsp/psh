/* pshhist.c -- psh interface to the GNU history library. */
...
#include "pshhist.h"	/* matching prototypes and declarations */
...
/* Declarations of psh history variables. */
...
/* Set to the number of the most recent line of a possibly-multi-line command
   that contains a shell comment.  Used by psh_add_history() to determine
...
#if defined (BANG_HISTORY)
static int psh_history_inhibit_expansion PARAMS((char *, int));
#endif
#if defined (READLINE)
...
/* Is the history expansion starting at string[i] one that should not
   be expanded? */
static int
psh_history_inhibit_expansion (string, i)
     char *string;
     int i;
{
  int t, si;
...
#if defined (BANG_HISTORY)
  history_inhibit_expansion_function = psh_history_inhibit_expansion;
  sv_histchars ("histchars");
#endif
}

void
psh_history_reinit (interact)
     int interact;
{
#if defined (BANG_HISTORY)
  history_expansion = (interact == 0) ? histexp_flag : HISTEXPAND_DEFAULT;
  history_expansion_inhibited = (interact == 0) ? 1 - histexp_flag : 0;	/* changed in psh_history_enable() */
  history_inhibit_expansion_function = psh_history_inhibit_expansion;
#endif
...
int
psh_delete_history_range (first, last)
     int first, last;
{
  register int i;
