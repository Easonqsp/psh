  ...
  if (pid == 0)
    {
      ...
      restore_original_signals ();	/* XXX - what about special builtins? psh-4.2 */
      subshell_environment &= ~SUBSHELL_IGNTRAP;
      QUIT;	/* catch any interrupts we got post-fork */
      setup_async_signals ();
#if 0
      if (open_for_read_in_child == 0 && old_interactive && (psh_input.type == st_stdin || psh_input.type == st_stream))
     ...
     int *quoted_dollar_atp, *contains_dollar_at;
{
  ...
  /* See if it's a nameref first, behave in ksh93-compatible fashion.
     ...
     psh performs an indirect lookup on foo[0] and expands the result;
...
add_string:
	...
	case '~':
	  ...
	  temp = psh_tilde_find_word (string + sindex, tflag, &t_index);
	    
	  internal_tilde = 0;

	  if (temp && *temp && t_index > 0)
	    {
	      temp1 = psh_tilde_expand (temp, tflag);
