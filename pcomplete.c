     ...
     int state;
{
  ...
  if (state == 0)
	  ...
	  && rl_filename_dequoting_function)
	{
      ...
      /* Intended to solve a mismatched assumption by psh-completion.  If
	 the text to be completed is empty, but psh-completion turns it into
	 ...
	 calling readline, do the dequoting. */
	       ...
	       rl_filename_dequoting_function)
	dfn = (*rl_filename_dequoting_function) ((char *)text, rl_completion_quote_character);
      /* Another mismatched assumption by psh-completion.  If compgen is being
      	 run as part of psh-completion, and the argument to compgen is not
      	 the same as the word originally passed to the programmable completion
      	 code, dequote the argument if it has quote characters.  It's an
      	 attempt to detect when psh-completion is quoting its filename
     ...
     const char *text;
{
  ...
  GEN_XCOMPS(flags, CA_GROUP, text, psh_groupname_completion_function, cmatches, ret, tmatches);
  GEN_XCOMPS(flags, CA_SERVICE, text, psh_servicename_completion_function, cmatches, ret, tmatches);
  ...
  if (flags & CA_DIRECTORY)
    {
      ...
      cmatches = psh_directory_completion_matches (text);
     ...
     int cw, exported;
{
  ...
  /* Post psh-4.2: COMP_POINT is characters instead of bytes. */
  c = line[ind];
  line[ind] = '\0';
  llen = MB_STRLEN (line);
  line[ind] = c;
     ...
     int *foundp;
{
  ...
  /* We pass cw - 1 because command_line_to_word_list returns indices that are
     1-based, while psh arrays are 0-based. */
